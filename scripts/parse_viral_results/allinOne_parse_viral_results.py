...
def main():	
	...
	* WebSite: http://www.dynatable.com/
	...
	# Create Dir
	if not os.path.exists("./viral_parser_output"):
		os.makedirs("./viral_parser_output")
	
	# Move HTML file to output dir
	shutil.move("viral_results.html", "./viral_parser_output/viral_results.html")
	
	# Move Json file to output dir
	shutil.move("viral_results.json", "./viral_parser_output/viral_results.json")
	
	# Move and rename final TSV file to output dir
	os.rename("TMPresultsFixedResults.txt", "./viral_parser_output/TSV_viral_results.txt")
	if Bool_metadata == 1:
		os.rename("TMPresults_fixed_added_metadata.txt", "./viral_parser_output/TSV_with_metaData_viral_results.txt")
